    ...
    STUDIP.Dialog.fromElement = function (element, options) {
        ...
        if ($(element).is('form,button,input')) {
            ...
            if ($(element).closest('form').attr('enctype') === 'multipart/form-data') {
                ...
                fd = new FormData();
                options.data.forEach(function (item) { fd.append(item.name, item.value) });
        ...
        dialog_options = $.extend(dialog_options, {
            ...
            open: function () {
                var helpbar_element = $('.helpbar a[href*="hilfe.studip.de"]'),
                    ...
                    element = $('<a class="ui-dialog-titlebar-wiki"'
                                + ' target="_blank" rel="noopener noreferrer">').attr('href', link).attr('title', tooltip),
