const News = {
    ...
    toggle_category_view: function(id) {
        ...
        if ($('#' + id + '_content').is(':visible')) {
            ...
            $('#' + id + ' input[type=image]:first').attr(
                'src',
                STUDIP.ASSETS_URL + 'images/icons/blue/arr_1right.svg'
            );
        } else {
            $('#' + id + '_content').slideDown(400);
            $('#' + id + ' input[type=image]:first').attr('src', STUDIP.ASSETS_URL + 'images/icons/blue/arr_1down.svg');
        }
    }
};
