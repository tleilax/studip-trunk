...
# Define public functions =================================================== #
     ...
     */
     public function save (&$evalObject)
     {
         ...
         if ($this->exists($evalObject->getObjectID ())) {
             DBManager::get()->execute(
                ...
                [
                    ...
                    $startdate, $stopdate, $timespan,
                    $evalObject->getCreationdate(), $evalObject->getChangedate(),
                    $evalObject->isAnonymous(), $evalObject->isVisible(), $evalObject->isShared(),
                    $evalObject->getObjectID()
                ]
            );
        } else {
            DBManager::get()->execute(
                ...
                [
                    $evalObject->getObjectID(),
                    $evalObject->getAuthorID(), $evalObject->getTitle(), $evalObject->getText(),
                    $startdate, $stopdate, $timespan,
                    $evalObject->getCreationdate(), $evalObject->getChangedate(),
                    $evalObject->isAnonymous(), $evalObject->isVisible(), $evalObject->isShared()
                ]
        ...
        // connect to ranges
        DBManager::get()->execute(
            "DELETE FROM eval_range WHERE eval_id = ?",
            [$evalObject->getObjectID()]
        );
    ...
    {
        // delete evaluation as well as connected ranges and users
        $query = "DELETE `eval`, `eval_range`, `eval_user`
                 FROM `eval`
                 LEFT JOIN `eval_range` USING (`eval_id`)
                 LEFT JOIN `eval_user` USING (`eval_id`)
                 WHERE eval_id = ?";
        DBManager::get()->execute($query, [$evalObject->getObjectID()]);
    }
    ...
    public function exists ($evalID)
    {
        return (bool) DBManager::get()->fetchColumn(
            "SELECT 1 FROM eval WHERE eval_id = ?",
            [$evalID]
        );
    }
    ...
    public function hasVoted ($evalID, $userID = "")
    {
        ...
        if (!$userID) {
            return (bool) DBManager::get()->fetchColumn($sql, [$evalID]);
        }
...
# Define static functions =================================================== #
     ...
     * @param    string   $userID   The user id
     */
    public static function connectWithUser ($evalID, $userID)
    {
        if (!$userID) {
            throw new Excpetion('EvaluationDB::connectWithUser: UserID leer!!');
        }
        DBManager::get()->execute(
            "INSERT IGNORE INTO eval_user SET eval_id = ?, user_id = ?",
            [$evalID, $userID]
        );
    }
    ...
    * @param    string   $userID   The user id
    */
    public static function removeUser($evalID, $userID = "")
    {
    ...
    public static function getNumberOfVotes($evalID)
    {
        return DBManager::get()->fetchColumn(
            "SELECT count(DISTINCT user_id) FROM eval_user WHERE eval_id = ?",
            [$evalID]
        );
    }
     ...
     * @return array
     */
    public static function search_range($search_str)
    {
