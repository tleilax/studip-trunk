...
class Dokument extends ModuleManagementModel
{
    ...
    {
        return parent::getEnrichedByQuery('SELECT md.* '
                . 'FROM mvv_dokument md '
            ...
            $row_count = null, $offset = null, $filter = null)
    {
        ...
        return parent::getEnrichedByQuery('SELECT mvv_dokument.*, '
    ...
    {
        $zuordnungen = array();
        $stmt = DBManager::get()->prepare('SELECT mdz.* '
    ...
    {
        $stmt = DBManager::get()->prepare('SELECT COUNT(range_id) '
    ...
    public static function updateDocuments($object, $dokument_ids, $annotations = array()) {
        ...
        foreach ((array) $dokument_ids as $dokument_id) {
            $dokument = $object->document_assignments
                    ->findOneBy('dokument_id', $dokument_id);
            if ($dokument) {
                $dokument->kommentar = trim($annotations[$dokument_id]['kommentar']);
                $dokument->kommentar_en = trim($annotations[$dokument_id]['kommentar_en']);
                $dokument->position = $pos++;
            } else {
                $dokument = new DokumentZuord();
    ...
    {
        $stmt = DBManager::get()->prepare('DELETE FROM mvv_dokument_zuord '
                . 'WHERE range_id = ? AND object_type = ?');
    ...
    {
        $query = 'SELECT dokument_id, name FROM mvv_dokument '
            . 'WHERE (name LIKE :input OR url LIKE :input) '
            . "AND dokument_id NOT IN ('"
            . implode("','", $exclude) . "') ORDER BY name ASC";
