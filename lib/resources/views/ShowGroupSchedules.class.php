        ...
        <form method="POST" name="schedule_form" action="<?= URLHelper::getLink('?navigate=TRUE&quick_view=view_group_schedule&quick_view_mode='.$view_mode) ?>">
            ...
            <tr>
                ...
                <td class="<? echo $cssSw->getClass() ?>" width="60%" valign="top">
                    <?=_("Eine Raumgruppe ausw‰hlen")?>:<br>
            ...
            foreach ($room_group->getGroupContent($this->group_id) as $room_to_show_id => $room_id){
                ...
                if ( ($end_time > time()) && ($_SESSION['resources_data']["show_repeat_mode"] == 'single' || $_SESSION['resources_data']["show_repeat_mode"] == 'all')){
                    ...
                    while ($event = $assign_events->nextEvent()) {
                        //mehrt‰gige nur am passenden Tag anzeigen
                        ...
                        if(in_array($event->repeat_mode, array('w','d','m','y'))){
                            ...
                            switch($event->repeat_mode){
                                ...
                                break;
                                case 'w':
                                if($assign->getRepeatInterval() == 1) $add_info = '('._("wˆchentlich").')';
                                else  $add_info = '('.$assign->getRepeatInterval().'-'._("wˆchentlich").')';
                                break;
                                case 'm':
        ...
        <table border=0 celpadding=2 cellspacing=0 width="99%" align="center">
            <tr>
                ...
                <td class="<? echo $cssSw->getClass() ?>" width="10%" align="center">&nbsp;
                    ...
                    <?= Icon::create('arr_2right', 'clickable')->asImg(16, ['class' => "middle", 'alt' => _("N‰chsten Tag anzeigen"), 'title' => _("N‰chsten Tag anzeigen")]) ?>
                </td>
            </tr>
            <tr>
                ...
                <? if ((!$_SESSION['resources_data']["schedule_time_range"]) || ($_SESSION['resources_data']["schedule_time_range"] == 1)): ?>
                    <a href="<?= URLHelper::getLink('', array('quick_view' => $this->used_view,
                        ...
                        <?= Icon::create('arr_2up', 'clickable')->asImg(16, ['class' => 'middle') + tooltip2(_('Fr¸here Belegungen anzeigen')]) ?>
                    </a>
                <? endif; ?>
                </td>
            ...
            <tr>
                ...
                <? if ((!$_SESSION['resources_data']['schedule_time_range']) || ($_SESSION['resources_data']['schedule_time_range'] == -1)): ?>
                    <a href="<?= URLHelper::getLink('', array('quick_view' => $this->used_view,
                                                              'quick_view_mode' => $view_mode,
                                                              'time_range' => $_SESSION['resources_data']['schedule_time_range'] ? 'FALSE' : 1)) ?>">
                        <?= Icon::create('arr_2down', 'clickable')->asImg(16, ['class' => 'middle') + tooltip2(_('Fr¸here Belegungen anzeigen')]) ?>
                    </a>
                <? endif; ?>
                </td>
