...
class ShowToolsRequests {
	...
	function showToolStart() {
		...
		<form method="POST" name="tools_requests_form" action="<?echo $PHP_SELF ?>?tools_requests_start=1">
			...
			<tr>
				...
				<?=makeButton("auswaehlen",'input',_("Semester ausw�hlen"),'tools_requests_sem_choose_button')?>
				</td>
	...
	function showRequestList() {
		...
		if ($license_to_kill){
			...
			echo chr(10) . '<div align="right">
				...
				. '</a>&nbsp;&nbsp;'
				. makeButton('loeschen', 'input', _("Ausgew�hlte Anfragen l�schen"), 'do_delete_requests')
				. '&nbsp;</div><br>';
		}
		$i = 0;
					...
					<table cellpadding="2" cellspacing="0" border="0" width="90%">
						...
						<tr>
							...
							if ($request_resource_id = $reqObj->getResourceId()) {
								...
								print "<img src=\"".$GLOBALS['ASSETS_URL']."images/info.gif\" ".tooltip(_("Der ausgew�hlte Raum bietet folgende der w�nschbaren Eigenschaften:")." \n".$resObj->getPlainProperties(TRUE), TRUE, TRUE)." />";
						...
						<tr>
							<td colspan="<?=$cols+2?>" align="center">
								<font size="-1">
									...
									<input type="IMAGE" name="matching_rooms_limit_submit" src="<?= $GLOBALS['ASSETS_URL'] ?>images/move_right.gif" border="0" <?=tooltip(_("ausgew�hlten Bereich anzeigen"))?>/>
								</font>
							</td>
						</tr>
							<?
	...
	function showGroupOverlapStatus($overlaps, $events_count, $overlap_events_count, $group_dates) {
		if ($overlap_events_count) {
			...
			$html = "<img src=\"".$GLOBALS['ASSETS_URL']."images/ampel_gruen.gif\" ".tooltip(_("Es existieren keine �berschneidungen"), TRUE, TRUE)."/>";
			$status = 0;
		}
