...
* @version	$Id: ExternModuleTemplateLecturedetails.class.php 6854 2006-10-18 16:04:09Z pthiene $
* @access		public
* @modulegroup	extern
			...
			if ($generic_datafields = $this->config->getValue('Main', 'genericdatafields')) {
				...
				foreach ($generic_datafields as $datafield) {
					if (isset($datafields[$datafield]['content'])) {
						$content['LECTUREDETAILS']["DATAFIELD_$k"] = ExternModule::ExtFormatReady($datafields[$datafield]['content'], TRUE, TRUE);
					}
					$k++;
				}
			}
			
			$content['__GLOBAL__']['STUDIP-EDIT-HREF'] = "{$GLOBALS['ABSOLUTE_URI_STUDIP']}seminar_main.php?auswahl={$this->seminar_id}&again=1&redirect_to=admin_seminare1.php&login=true&new_sem=TRUE";
