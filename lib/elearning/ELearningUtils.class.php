        ...
        foreach ($cmsystems as $cms_type =>$data) {
            if ($ELEARNING_INTERFACE_MODULES[$cms_type]) {
                ...
                if (ELearningUtils::getConfigValue("ACTIVE", $cms_type)) {
                    $output .= "<tr><td>" .  Icon::create('checkbox-checked', 'clickable')->asImg(16, ['class' => 'text-top']) . "</td><td><b>". sprintf(_("Die Schnittstelle zum System %s ist aktiv."), $ELEARNING_INTERFACE_MODULES[$cms_type]["name"]) . "</b></td></tr>";
                    $output .= "<tr><td colspan=\"2\">&nbsp;</td></tr>";
                }
                elseif ($data["config"] < 1)
                    $output .= "<tr><td>" .  Icon::create('checkbox-unchecked', 'clickable')->asImg(16, ['class' => 'text-top']) . "</td><td><i>". sprintf(_("Die Schnittstelle für das System %s wurde noch nicht eingerichtet."), $ELEARNING_INTERFACE_MODULES[$cms_type]["name"]) . "</i></td></tr>";
                elseif ($data["config"] < 1)
                    $output .= "<tr><td>" .  Icon::create('checkbox-unchecked', 'clickable')->asImg(16, ['class' => 'text-top']) . "</td><td><i>". sprintf(_("Die Schnittstelle wurde noch nicht aktiviert."), $ELEARNING_INTERFACE_MODULES[$cms_type]["name"]) . "</i></td></tr>";
                ...
                $output .= "<table>";
                $output .= "<tr><td colspan=\"2\">&nbsp;</td></tr>";
                $output .= "<tr><td>" . Icon::create('decline', 'attention')->asImg(16, ['class' => 'text-top']) . "</td><td><i>".sprintf(_("Für das System mit dem Index \"%s\" existieren keine Voreinstellungen in den Konfigurationsdateien mehr."), $cms_type) . "</i></td></tr>";
                $output .= "<tr><td colspan=\"2\">&nbsp;</td></tr>";
