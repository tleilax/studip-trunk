...
if ($th_title) {
    ?>
    <tr>
        <th colspan="<?= sizeof($structure) - ($structure['actions'] ? 1 : 0) ?>">
            <?= htmlReady($th_title) ?>
        </th>
        <? if ($structure['actions']) : ?>
            <th class="actions">
                <? if ($mail_status) {
                    ?>
                    <a data-dialog
                       href="<?= URLHelper::getScriptLink("dispatch.php/messages/write", ['filter' => 'inst_status', 'who' => $key, 'default_subject' => $GLOBALS['SessSemName'][0], 'course_id' => $GLOBALS['SessSemName'][1]]) ?>">
                    ...
                    ?>
                    <a data-dialog
                       href="<?= URLHelper::getScriptLink("dispatch.php/messages/write", ['group_id' => $role_id, 'default_subject' => $GLOBALS['SessSemName'][0]]) ?>">
...
foreach ($members as $member) {
    ...
    $default_entries = DataFieldEntry::getDataFieldEntries([$member['user_id'], $range_id]);

    if ($member['statusgruppe_id']) {
        $role_entries = DataFieldEntry::getDataFieldEntries([$member['user_id'], $member['statusgruppe_id']]);
    }
    ...
    if ($structure["statusgruppe"]) {
        $statusgroups = new SimpleCollection(StatusgruppeUser::findBySQL('user_id = ? AND statusgruppe_id IN (?)', [$member['user_id'], array_keys((array)$group_list)]));
        if (count($statusgroups)) {
            $statusgroups = $statusgroups->pluck('statusgruppe_id');
            foreach ($statusgroups as $statusgroup) {
                $entries = DataFieldEntry::getDataFieldEntries([$member['user_id'], $id]);
