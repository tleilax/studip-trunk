...
<tr>
    ...
    <th width="5%" align="right" colspan="3" nowrap>
        <? if ($role['role']->hasFolder()) :
            echo Icon::create('files', 'inactive', ['title' => _("Dateiordner vorhanden")])->asImg(16);
        endif; ?>
        <? if ($role['role']->getSelfAssign()) :
            echo Icon::create('person', 'inactive', ['title' => _("Personen können sich dieser Gruppe selbst zuordnen")])->asImg(16);
        endif; ?>
        <?= $mp; ?>
        <a href="<?= URLHelper::getLink('?cmd=editRole&role_id='.  $id) ?>">
            <?= Icon::create('edit', 'clickable', ['title' => _("Gruppe bearbeiten")])->asImg(16) ?>
        </a>
        <a href="<?= URLHelper::getLink('?cmd=sortByName&role_id='.  $id) ?>">
            <?= Icon::create('arr_eol-down', 'clickable', ['title' => _("Personen dieser Gruppe alphabetisch sortieren")])->asImg(16) ?>
        </a>
    </th>
    <th width="1%" align="right">
        <a href="<?= URLHelper::getLink('?cmd=deleteRole&role_id='. $id) ?>">
            <?= Icon::create('trash', 'attention', ['title' => _("Gruppe mit Personenzuordnung entfernen")])->asImg(16) ?>
        </a>
    </th>
</tr>
<?
...
<tr>
    ...
    <td width="1%" nowrap>
        ...
        <a href="<?= URLHelper::getLink('?cmd=move_down&role_id='. $id .'&username='. $person['username']) ?>">
            <?= Icon::create('arr_2down', 'sort', ['title' => _("Person eine Position nach unten platzieren")])->asImg(16) ?>
        </a>
        <? endif; ?>
    </td>
    ...
    <td width="1%" nowrap style="padding-left: 4px">
        ...
        <a href="<?= URLHelper::getLink('?cmd=move_up&role_id='. $id .'&username='. $person['username']) ?>">
            <?= Icon::create('arr_2up', 'sort', ['title' => _("Person einen Position nach oben platzieren")])->asImg(16) ?>
        </a>
        <? endif; ?>
    </td>
    ...
    <td width="1%" align="right">
        ...
        <?= Icon::create('trash', 'clickable', ['title' => _("Gruppenzuordnung für diese Person aufheben")])->asImg(16) ?>
        </a>
    </td>
</tr>
<? endforeach; ?>
