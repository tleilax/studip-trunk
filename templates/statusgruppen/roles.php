...
<tr>
    ...
    <td class="printhead" valign="bottom" colspan="<?= 19-$indent ?>" height="22" nowrap style="padding-left: 3px" width="<?= 99-$indent ?>%">
        <a name="<?= $id ?>">
        <? if ($open == $id) : ?>
        <a class="tree" href="<?= URLHelper::getLink('?list=true#'. $id) ?>"><?= Icon::create('arr_1down', 'clickable')->asImg(16, ['class' => 'text-top']) ?></a>
        <? else : ?>
        <a class="tree" href="<?= URLHelper::getLink('?role_id='. $id .'#'. $id) ?>"><?= Icon::create('arr_1right', 'clickable')->asImg(16, ['class' => 'text-top']); ?></a>
        <? endif; ?>

        <? if ($move) : ?>
        <a href="#"><?= Icon::create('arr_2right', 'sort')->asImg(16) ?></a>
        <? endif; ?>

        <? if ($sort) :
            if ($pos > 1) : ?>
        <a href="<?= URLHelper::getLink('?cmd=moveUp&view=sort&role_id='. $id) ?>"><?= Icon::create('arr_2up', 'sort')->asImg(16); ?></a>
        <? endif; if ($pos < sizeof($roles)) : ?>
        <a href="<?= URLHelper::getLink('?cmd=moveDown&view=sort&role_id='. $id) ?>"><?= Icon::create('arr_2down', 'sort')->asImg(16); ?></a>
        <? endif;
        endif;
        ?>
            <? if ($open == $id) : ?>
    ...
    <td width="1%" class="printhead" align="right" valign="bottom" nowrap>
        <? if ($role['role']->hasFolder()) :
            echo Icon::create('files', 'clickable')->asImg(16);
        endif; ?>
