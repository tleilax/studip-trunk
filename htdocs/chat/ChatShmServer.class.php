...
require_once $ABSOLUTE_PATH_STUDIP . $RELATIVE_PATH_CHAT."/ShmHandler.class.php";
...
class ChatShmServer {
	...
	function getActiveUsers($rangeid){
		...
		foreach($this->chatUser as $userid => $detail){
			if (($a_time-$detail[$rangeid]["action"]) > CHAT_IDLE_TIMEOUT)
	...
	function addUser($userid,$rangeid,$nick,$fullname,$chatperm){
		...
		if (!$this->chatUser[$userid][$rangeid]["color"])
			$this->chatUser[$userid][$rangeid]["color"]="black";
