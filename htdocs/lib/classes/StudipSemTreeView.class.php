	...
	*/
	...
	function getItemContent($item_id){
		...
		if ($this->tree->tree_data[$item_id]['info']){
			$content .= "\n<tr><td class=\"blank\" align=\"left\" colspan=\"2\">";
			$content .= formatReady($this->tree->tree_data[$item_id]['info']) . "</td></tr>";
		}
		$content .= "<tr><td colspan=\"2\" class=\"blank\">" . sprintf(_("Alle Veranstaltungen innerhalb dieses Bereiches in der %s&Uuml;bersicht%s"),
		...
		if ($this->tree->getNumEntries($item_id) - $this->tree->tree_data[$item_id]['lonely_sem']){
			$content .= "<tr><td class=\"blank\" align=\"left\" colspan=\"2\"><b>" . _("Eintr&auml;ge auf dieser Ebene:");
			$content .= "</b>\n</td></tr>";
			$entries = $this->tree->getSemData($item_id);
			$content .= $this->getSemDetails($entries->getGroupedResult("seminar_id"));
		} else {
			$content .= "\n<tr><td class=\"blank\" colspan=\"2\">" . _("Keine Eintr&auml;ge auf dieser Ebene vorhanden!") . "</td></tr>";
		}
		if ($this->tree->tree_data[$item_id]['lonely_sem']){
			$content .= "<tr><td class=\"blank\" align=\"left\" colspan=\"2\"><b>" . _("Nicht zugeordnete Veranstaltungen auf dieser Ebene:");
			$content .= "</b>\n</td></tr>";
