	...
	global $ADODB_CACHE_DIR;
		...
		if (strlen($ADODB_CACHE_DIR) > 1 && !$sql) {
			if (strncmp(PHP_OS,'WIN',3) === 0) {
				...
				$dh  = opendir($ADODB_CACHE_DIR); 
				while (false !== ($filename = readdir($dh))) { 
					if(!fnmatch('adodb_*.cache', $filename)) continue; 
					unlink($ADODB_CACHE_DIR.'/'.$filename); 
					if ($this->debug) $deb[] = 'unlink(' . $ADODB_CACHE_DIR . '/' . $filename . ');'; 
				}
			} else {
			...
			if ($this->debug) {
				if ($cmd) ADOConnection::outp( "CacheFlush: $cmd<br><pre>\n", system($cmd),'</pre>');
				else ADOConnection::outp( "CacheFlush: PHP safe_mode<br><pre>\n", implode("\n", $deb), '</pre>');
			} elseif($cmd) {
				exec($cmd);
			}
			return;
