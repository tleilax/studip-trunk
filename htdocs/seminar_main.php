...
IF ($SessSemName[1] =="")
	...
	$db=new DB_Seminar;
	$db->query ("SELECT seminar_user.user_id, Vorname, Nachname, username, status FROM auth_user_md5 LEFT JOIN seminar_user USING (user_id) WHERE seminar_user.Seminar_id = '$SessionSeminar' AND status = 'dozent' ORDER BY Nachname");
