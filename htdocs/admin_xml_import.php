...
function lies_seminar()
{
	...
	while (((strpos($zeile, "/" . "Lecture") ===false)) AND (!feof($datei)))
	{
		...
		$zeile = fgets($datei, 4096);
		$zeile = str_replace("&#xFC;", "Å", $zeile);
		$zeile = str_replace("&#xE4;", "Ñ", $zeile);
...
function get_objects($string)
{
	...
	while((!feof($datei)) AND (strpos($zeile, $string . " key") ===false))
	{
		...
		elseif (strpos($zeile, "orgname") >0)
		{
			...
			$zeile = str_replace("&#xE4;", "Ñ", $zeile);
			$zeile = str_replace("&#xF6;", "˜", $zeile);
			$zeile = str_replace("&#xDC;", "ö", $zeile);
			$zeile = str_replace("&#xC4;", "é", $zeile);
			$zeile = str_replace("&#xD6;", "ô", $zeile);
			$zeile = str_replace("&#xDF;", "ò", $zeile);
...
if ($assi_page == 5)
{
	...
	while (list ($key, $val) = each ($xml_import_person)) 
	{
		if (($i >= $person_nr) AND ($i < $person_nr+10))
		{
			...
			$uname = str_replace("Ñ","ae",$uname);
			$uname = str_replace("˜","oe",$uname);
			$uname = str_replace("Å","ue",$uname);
			$uname = str_replace("ò","ss",$uname);
