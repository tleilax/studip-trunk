...
function show_content() {
	$data = "<br><a href=\"".$PHP_SELF."?action=new\">"._("<b>Neue Sperrebene anlegen</b>")."</a>";
	...
	$data .= $zt->cell("<b>"._("")."</b>",array("width"=>"10%"));
	$data .= $zt->cell("<b>"._("")."</b>",array("width"=>"10%"));
	$data .= $zt->closeRow();
	$all_lock_data = show_lock_rules();
	if (is_array($all_lock_data)) {
		for ($i=0;$i<count($all_lock_data);$i++) {
			$data .= $zt->row(array($all_lock_data[$i]["name"], $all_lock_data[$i]["description"], "<a href=".$PHP_SELF."?action=edit&lock_id=".$all_lock_data[$i]["lock_id"]."><img ".makeButton("bearbeiten","src")." border=0></a>", "<a href=".$PHP_SELF."?action=confirm_delete&lock_id=".$all_lock_data[$i]["lock_id"]."><img ".makeButton("loeschen","src")." border=0></a>"));
		}
	}
	$data .= $zt->close();
	return $data;
...
function show_lock_rule_form($lockdata="",$edit=0) {
	...
	$form .= $zt->cell("<b>"._("")."</b>",array("width"=>"14%","align"=>"left"));
	$form .= $zt->cell("<b>"._("")."</b>",array("width"=>"13%","align"=>"left"));
	//$form .= $zt->closeRow();
	//$form .= $zt->row(array("<font size=4><b>"._("Attribut")."</b></font>","",""));
	$form .= $zt->row(array("<font size=3><b>"._("Grunddaten")."</b></font>","<font size=3><b>"._("gesperrt")."</b></font>","<font size=3><b>"._("nicht gesperrt")."</b></font>"));
	...
	$form .= $zt->row(array("<font size=3><b>"._("Personen und Einordnung")."</b></font>","<font size=3><b>"._("gesperrt")."</b></font>","<font size=3><b>"._("nicht gesperrt")."</b></font>"));
	...
	$form .= $zt->row(array("<font size=3><b>"._("weitere Daten")."</b></font>","<font size=3><b>"._("gesperrt")."</b></font>","<font size=3><b>"._("nicht gesperrt")."</b></font>"));
	...
	if ($lockdata["attributes"]["vorraussetzungen"]) {
		$form .= $zt->row(array(_("Teilnahmevoraussetzungen"),"<input type=\"radio\" name=\"lockdata[attributes][vorraussetzungen]\" value=1 checked>","<input type=\"radio\" name=\"lockdata[attributes][vorraussetzungen]\" value=0>"));	
	} else {
		$form .= $zt->row(array(_("Teilnahmevoraussetzungen"),"<input type=\"radio\" name=\"lockdata[attributes][vorraussetzungen]\" value=1>","<input type=\"radio\" name=\"lockdata[attributes][vorraussetzungen]\" value=0 checked>"));	
	}
	...
	for ($i=1;$i<count($datafields);$i++) {
		if ($lockdata["attributes"][$datafields[$i]["id"]]) {
			$form .= $zt->row(array($datafields[$i]["name"],"<input type=\"radio\" name=\"lockdata[attributes][".$datafields[$i]["id"]."]\" value=\"1\" checked>","<input type=\"radio\" name=\"lockdata[attributes][".$datafields[$i]["id"]."]\" value=\"0\">"));
		} else {
			$form .= $zt->row(array($datafields[$i]["name"],"<input type=\"radio\" name=\"lockdata[attributes][".$datafields[$i]["id"]."]\" value=\"1\">","<input type=\"radio\" name=\"lockdata[attributes][".$datafields[$i]["id"]."]\" value=\"0\" checked>"));
		}
	}
	$form .= $zt->row(array("<font size=3><b>"._("Zeiten")."</b></font>","<font size=3><b>"._("gesperrt")."</b></font>","<font size=3><b>"._("nicht gesperrt")."</b></font>"));
	...
	$form .= $zt->row(array("<font size=3><b>"._("Zugangsberechtigungen")."</b></font>","<font size=3><b>"._("gesperrt")."</b></font>","<font size=3><b>"._("nicht gesperrt")."</b></font>"));
	...
	if ($edit) {
		...
		$form .= $zt->openRow();
		$form .= $zt->cell("<input type=\"IMAGE\" ".makeButton("uebernehmen", "src").">&nbsp;<a href=\"$PHP_SELF\"><img ".makeButton("abbrechen","src")." border=0></a>",array("colspan"=>"3","align"=>"center"));
	} else {
		$form .= "<input type=\"hidden\" name=\"action\" value=\"insert\">";
		$form .= $zt->openRow();
		$form .= $zt->cell("<input type=\"IMAGE\" ".makeButton("anlegen", "src").">&nbsp;<a href=\"$PHP_SELF\"><img ".makeButton("abbrechen","src")." border=0></a>",array("colspan"=>"3","align"=>"center"));
	}
	$form .= "</form>";
	$form .= $zt->close();
	return $form;
