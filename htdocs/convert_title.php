...
if($_REQUEST['convertible']){
	...
	$query = "SELECT user_id, LEFT(Vorname,INSTR(Vorname,SUBSTRING_INDEX(Vorname,' ',-1))-1) AS title,SUBSTRING_INDEX(Vorname,' ',-1) AS new_vorname
			FROM auth_user_md5 WHERE user_id IN('".join("','",$conv)."')";
	$db->query($query);
	while($db->next_record()){
		$db1->query("UPDATE user_info SET title='".trim($db->f("title"))."' WHERE user_id='".$db->f("user_id")."'");
...
$query = "SELECT user_id,username, LEFT(Vorname,INSTR(Vorname,SUBSTRING_INDEX(Vorname,' ',-1))-1) AS title,SUBSTRING_INDEX(Vorname,' ',-1) AS new_vorname,Vorname,Nachname FROM auth_user_md5 WHERE (LEFT(Vorname,INSTR(Vorname,SUBSTRING_INDEX(Vorname,' ',-1))-1))!='' ORDER BY LEFT(Vorname,INSTR(Vorname,SUBSTRING_INDEX(Vorname,' ',-1))-1)";
$db->query($query);
echo "<form action=\"$PHP_SELF\" method=\"post\">"; 
echo "<table  border=\"0\" cellspacing=\"2\" cellpadding=\"2\"><tr>";
echo "<th>username</th><th>Vorname</th><th>Nachname</th><th>new_vorname</th><th>titel</th><th><a href=\"#\" onClick=\"invert_selection();\" title=\"Auswahl umkehren\">umwandeln ?</a></th></tr>";
while ($db->next_record()){
	...
	echo"<td class=\"blank\" align=\"center\">".$db->f("title")."</td>";
