...
{	global $SessSemName, $PHP_SELF;
	...
	while ($db->next_record()) {
		...
		printf ( "	          <td width=\"5%%\"><a href=\"$PHP_SELF?cmd=remove_statusgruppe&statusgruppe_id=%s\"><img src=\"pictures/lighttrash.gif\" width=\"11\" height=\"17\" border=\"0\"></a></td>",$statusgruppe_id);
		echo 	"</tr>";
		...
		while ($db2->next_record()) {
			...
			printf ( "	   <td width=\"5%%\"><a href=\"$PHP_SELF?cmd=remove_person&statusgruppe_id=%s&username=%s\"><img src=\"pictures/trash.gif\" width=\"11\" height=\"17\" border=\"0\"></a></td>", $statusgruppe_id, $db2->f("username"));
			echo "	</tr>";
			$k++;
		}
		while ($k <= $db->f("size")) {
		...
		if ($i < $AnzahlStatusgruppen) {
			printf ("<p align=\"center\"><a href=\"$PHP_SELF?cmd=swap&statusgruppe_id=%s\"><img src=\"pictures/move_up.gif\" width=\"13\" height=\"11\" border=\"0\"></a>",$statusgruppe_id); 
			printf ("&nbsp; &nbsp; &nbsp; <a href=\"$PHP_SELF?cmd=swap&statusgruppe_id=%s\"><img src=\"pictures/move_down.gif\" width=\"13\" height=\"11\" border=\"0\"></a> </p>",$statusgruppe_id);
		}
	}
}
...
// Abfrage der Formulare und Aktionen
	...
	if (($cmd=="add_new_statusgruppe") && ($new_statusgruppe_name != "")) {
		...
		$chdate = time();
		$db=new DB_Seminar;
		$db->query ("SELECT position FROM statusgruppen WHERE range_id = '$SessSemName[1]' ORDER BY position DESC");
		if ($db->next_record()) {
			$position = $db->f("position")+1;
		} else {
			$position = "1";
		}
		$db->query("INSERT INTO statusgruppen SET statusgruppe_id = '$statusgruppe_id', name = '$new_statusgruppe_name', range_id= '$SessSemName[1]', position='$position', size = '$new_statusgruppe_size', mkdate = '$mkdate', chdate = '$chdate'");
...
//			echo "<b>$key</b>: $val<br>";
		...
		if ($InstitutMembers != "---") {
			...
			if (!$db->next_record()) {			
				...
				$db->query("INSERT INTO seminar_user SET Seminar_id = '$SessSemName[1]', user_id = '$user_id', status = 'autor', gruppe = '6' , mkdate = '$mkdate'");
			}
		}
		if ($Freesearch != "") {
			for ($i  = 0; $i < sizeof($Freesearch); $i++) {
				...
				if (!$db->next_record()) {
					$db2->query("INSERT INTO statusgruppe_user SET statusgruppe_id = '$statusgruppe_id', user_id = '$user_id'");
					$db2->query("INSERT INTO seminar_user SET Seminar_id = '$SessSemName[1]', user_id = '$user_id', status = 'autor', gruppe = '6' , mkdate = '$mkdate'");
				}
			}
		}
	...
	if ($cmd=="remove_statusgruppe") {
		...
		$db->query("SELECT * FROM statusgruppen WHERE range_id = '$SessSemName[1]' AND position > '$position'");
		while ($db->next_record()) {
	...
	if ($cmd=="swap") {
		...
		if ($db->next_record()) {
			...
			$db2->query("UPDATE statusgruppen SET position =  '$current_position' WHERE range_id = '$SessSemName[1]' AND position = '$next_position' AND statusgruppe_id != '$statusgruppe_id'");
		}
	}
