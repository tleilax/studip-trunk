...
function createDayTable($day_obj, $start = 6, $end = 19, $step = 900, $precol = TRUE,
	...
	for($i = $start / $step;$i < $end / $step + 3600 / $step;$i++){
		...
		if(!$term[$zeile]){
			...
			$height = "";
			// Wenn bereits hier ein Link eingefügt wurde braucht weiter unten keine
			// zusätliche Spalte ausgegeben werden
			$link_notset = FALSE;
		}
		else{
			if($colsp[$zeile] > 0)
...
// Tabellenansicht der Termine für eine Woche
function createWeekTable($week_obj, $start = 6, $end = 21, $step = 3600, $compact = TRUE, $link_edit = FALSE){
	...
	// Wochentag und Datum als Spaltenüberschriften
