...
<?
	...
	// bei Einsprung ohne $cmd wird weiter unten eine Erläuterung ausgegeben
	if(!$cmd && !$atime)
		$intro = TRUE;
	
	// Wird kein timestamp an das Skript übergegen, benutze aktuellen
	if(!$atime && !$termin_id)
		$atime = time();
	...
	// Datum für "Gehe-zu-Funktion" checken
	...
	if($cmd == ""){
		if($termin_id)
			// wird eine termin_id übergeben immer in den Bearbeiten-Modus
			$cmd = "edit";
		else
	...
	switch($cmd){
		case "showday":
			...
			$title = "Mein persönlicher Terminkalender - Tagesansicht";
			break;
		case "add":
			switch($calendar_sess_control_data["view_prv"]){
				case "showday":
					...
					break;
				case "showweek":
					$title = "Mein persönlicher Terminkalender - Wochenansicht";
					break;
				case "showmonth":
					$title = "Mein persönlicher Terminkalender - Monatsansicht";
					break;
				case "showyear":
					$title = "Mein persönlicher Terminkalender - Jahresansicht";
			}
			break;
		case "del":
			$title = "Mein persönlicher Terminkalender - Tagesansicht";
			$atermin = new Termin($termin_id);
			$atermin->delete();
		...
		case "showyear":
			...
			break;
		case "bind":
			$title = "Mein persönlicher Terminkalender - Seminartermine einbinden";
			break;
/*		case "import":
			$title = "Mein persönlicher Terminkalender - Termine importieren";
			break; */
		case "edit":
			...
			if($termin_id && !$mod){
				...
				$translate = array("SINGLE"=>"keine", "DAYLY"=>"täglich", "WEEKLY"=>"wöchentlich",
			...
			if($termin_id)
				$title = "Mein persönlicher Terminkalender - Termin bearbeiten";
			else
				$title = "Mein persönlicher Terminkalender - Neuer Termin";
	...
	if($cmd == "add"){
		...
		if(ceil((mktime(12,0,0,$end_month,$end_day,$end_year) - mktime(12,0,0,$start_month,$start_day,$start_year)) / 86400) > 2){
			$err["end_time"] = TRUE;
			$err_message = "<br>Ein Termin darf sich über max. zwei Tage erstrecken!";
		}*/
		...
		if(empty($err)){
			$atermin = new Termin($start,$end,$txt,$exp,$cat,$priority,$loc);		
			...
			// wird eine termin_id übergeben, werden nur die Daten des Termins geändert
			if($termin_id)
...
<table width="100%" cellspacing="0" cellpadding="0" border="0">
	...
	<tr><td class="blank">&nbsp;
		<blockquote>
			Dieser Terminkalender verwaltet Ihre Termine. Sie k&ouml;nnen Termine eintragen, ändern, 
			gruppieren und sich übersichtlich anzeigen lassen.
		</blockquote>
	</td></tr>
		<? }
		else
...
<table width="100%" border="0" cellpadding="5" cellspacing="0">
	...
	<tr><td class="blank" width="100%">
		...
		<?
			echo $aday->getName("LONG") . ", " . $aday->getDate();
...
<?
		// Im Kalenderblatt ist links oben immer Montag. Das muß natürlich nicht der
		// Monatserste sein. Es müssen evtl. noch Tage des vorangegangenen Monats
		// am Anfang und des folgenden Monats am Ende angefügt werden.
		
		$adow = strftime("%u", $amonth->getStart()) - 1;
		
		$first_day = $amonth->getStart() - $adow * 86400 + 43200;
		// Ist erforderlich, um den März richtig darzustellen
...
<?
	...
	// ist $termin_id an das Skript übergeben worden, dann bearbeite diesen Termin
	// ist $atime an das Skript übergeben worden, dann erzeuge neuen Termin (s.o.)
	if($cmd == "edit"){
		?>	
			<table width="100%" border="0" cellpadding="5" cellspacing="0">
				<tr><td class="blank" width="100%">
		...
		// Aufruf aus Tagesansicht für neuen Termin
		if($atime && !$termin_id && !$mod){
			...
			$cat = 1;
			$vue = "private";
			echo "<b>Termin erstellen für " . ldate($atime) . "</b></td></tr>\n";
		}
		// Aufruf aus Ansichten bestehenden Termin bearbeiten
			...
			if(check_date($start_month, $start_day, $start_year))
				echo "<b>Termin erstellen für " . ldate(mktime(0,0,0,$start_month,$start_day,$start_year)) . "</b></td></tr>\n";
		else{
			page_close();
			die;
		}
		...
		// Start- und Endzeit nur auf fünf Minuten genau einstellbar	
...
?>
			...
			<tr><td class="steel1">
				...
				<p>Veranstaltungstermine können nicht im persönlichen Terminkalender bearbeitet werden.</p>
<?
...
?>
			...
			<tr><td class="steel1" valign="middle">
			<? if($repeat["type"] == "DAYLY" || $mod == "täglich")
					echo '<img src="./pictures/forumgrau.gif" border="0" alt="Dieser Termin wird täglich wiederholt">';
				 else
			...
			<tr><td class="steel1" valign="middle">
			<? if($repeat["type"] == "WEEKLY" || $mod == "wöchentlich")
					echo '<img src="./pictures/forumgrau.gif" border="0" alt="Dieser Termin wird wöchentlich wiederholt">';
				 else
					echo '<img src="./pictures/forumleer.gif" border="0" alt="">'; ?>
			...
			<tr><td class="steel1" valign="middle">
			<? if($repeat["type"] == "YEARLY" || $mod == "jährlich")
					echo '<img src="./pictures/forumgrau.gif" border="0" alt="Dieser Termin wird jährlich wiederholt">';
				 else
					echo '<img src="./pictures/forumleer.gif" border="0" alt="">'; ?>
...
<?
	...
	<?
		...
		// Dummy-Wert damit $sem auch ohne ausgewählte Seminare ausgewertet wird
