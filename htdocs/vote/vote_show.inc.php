...
function show_votes($rangeID, $userID, $perm, $isHomepage = NO) {
	...
	if ($perm->have_studip_perm("tutor", $rangeID) || get_username($userID) == $rangeID)
		$haveFullPerm = true;
	else
		$haveFullPerm = false;
	...
	if ($voteDB->isError()) {
		echo createErrorReport($voteDB, _("Datenbankfehler bei Voteaktivierung"));
	}
	/* ---------------------------------------------------------------------- */
	...
	if ($evalDB->isError()) {
		echo createErrorReport($evalDB, _("Datenbankfehler beim Auslesen der EvaluationsIDs."));
	}
	...
	if (empty ($activeVotes) && empty ($stoppedVotes) && empty ($activeEvals) && empty ($stoppedEvals) && !($perm->have_studip_perm("tutor", $rangeID) || get_username($userID) == $rangeID)) {
		$voteDB->finalize();
		return;
	}
	/* ---------------------------------------------------------------------- */
	...
	if ($rangeID == "studip") {
		...
		if (count($activecourseEvals) > 0) {
			if ($perm->have_studip_perm("tutor", $rangeID) OR get_username($userID) == $rangeID) {
				echo createBoxHeader(_("Lehrveranstaltungsevaluationen zu meinen Veranstaltungen"), $width, "", VOTE_ICON_BIG, _("Evaluationen..."), EVAL_FILE_ADMIN .
				"?page=overview&rangeID=" . $rangeID .
				 ($GLOBALS['SessSemName']["class"] == "sem" ? "&new_sem=TRUE&view=vote_sem" : "&new_inst=TRUE&view=vote_inst"), VOTE_ICON_ARROW, _("Evaluationen bearbeiten"));
			} else {
				echo createBoxHeader(_("Lehrveranstaltungsevaluationen zu meinen Veranstaltungen"), $width, "", VOTE_ICON_BIG, _("Evaluationen..."));
			}
		}
			...
			foreach ($activecourseEvals as $semarray) {
				foreach ($semarray[0] as $evalID) {
					...
					if ($open) {
						...
						$maxTitleLength = ($isHomepage) ? VOTE_SHOW_MAXTITLELENGTH : VOTE_SHOW_MAXTITLELENGTH - 10;
	...
	if (count($activeEvals) + count($stoppedEvals) > 0) {
		if ($perm->have_studip_perm("tutor", $rangeID) OR get_username($userID) == $rangeID)
			echo createBoxHeader(_("Evaluationen"), $width, "", VOTE_ICON_BIG, _("Evaluationen..."), EVAL_FILE_ADMIN .
			"?page=overview&rangeID=" . $rangeID .
			 ($GLOBALS['SessSemName']["class"] == "sem" ? "&new_sem=TRUE&view=vote_sem" : "&new_inst=TRUE&view=vote_inst"), VOTE_ICON_ARROW, _("Evaluationen bearbeiten"));
		else
			echo createBoxHeader(_("Evaluationen"), $width, "", VOTE_ICON_BIG, _("Evaluationen..."));
	...
	foreach ($activeEvals as $evalID) {
		...
		/*
		 * TODO: trennen von votes und umfragen/tests
		 * Aufklappen aller evaluationen auch mit openAllVotes
		 */
		/* Get post and get-variables ---------------------------------------- */
		$formID = $_REQUEST["voteformID"];
		$openID = $_REQUEST["voteopenID"];

		$open = (($openID == $evalID) || $_GET['openAllEvals']) && (!$_GET['closeEvals']);
		/* ------------------------------------------------------------------- */
	...
	if (count($activeEvals) + count($stoppedEvals) > 1) {
		// create eval arrow (true param)
		echo createOpeningOrClosingArrow(TRUE);
		echo ("</td></tr></table>");
		echo ("<br>");
	} else {
		echo ("</td></tr></table>");
	...
	if ($perm->have_studip_perm("tutor", $rangeID) OR get_username($userID) == $rangeID)
		echo createBoxHeader(_("Umfragen"), $width, "", VOTE_ICON_BIG, _("Umfragen und mehr..."), VOTE_FILE_ADMIN .
		"?page=overview&rangeID=" . $rangeID .
		 ($GLOBALS['SessSemName']["class"] == "sem" ? "&new_sem=TRUE&view=vote_sem" : "&new_inst=TRUE&view=vote_inst"), VOTE_ICON_ARROW, _("Umfragen bearbeiten"));
	else
		echo createBoxHeader(_("Umfragen"), $width, "", VOTE_ICON_BIG, _("Umfragen und mehr..."));
	...
	foreach ($activeVotes as $tmpVote) {
		...
		if (!$previewResults)
			$previewResults = $_GET["previewResults"];
		...
		if ($open) {
			...
			if ($_GET["voteaction"] == "saved" && $voteID == $_GET["voteID"])
				echo createReportMessage(_("Die &Auml;nderungen wurden gespeichert"), VOTE_ICON_SUCCESS, VOTE_COLOR_SUCCESS) .
				"<br>\n";

			/* User has already used the vote --------------------------------- */
			if ($voteDB->isAssociated($voteID, $userID) && (!$changeAnswer) && (!$answerChanged)) {
			...
			/* User has just voted ------------------------------------------- */
			elseif (($voted && $formID == $voteID && !$changeAnswer) || ($voted && $formID == $voteID && $answerChanged)) {
	...
	if (empty ($activeVotes) AND empty ($stoppedVotes) AND empty ($activeEvals) AND (empty ($stoppedEvals) && $haveFullPerm)) {
