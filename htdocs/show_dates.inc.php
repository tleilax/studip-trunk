<?
/*
show_dates.inc.php enthÑlt Funktionen zum Anzeigen von Terminen
Copyright (C) 2000 AndrÇ Noack <anoack@mcis.de>, Cornelis Kater <ckater@gwdg.de>,
...
{ Global $PHP_SELF, $SessSemName, $user, $TERMIN_TYP, $username;	
	...
	if ($show_admin) {
		if ($range_id == $user->id)
			// FÅr pers˜nliche Termine Einsprung in Terminkalender
	...
	if ($db->num_rows()) {
		...
		if ($show_admin) {
			...
			if (!$show_whole_time)
				echo "\n<tr valign='baseline'><td class='topic'>&nbsp;<img src='./pictures/icon-uhr.gif' border='0' alt='Termine. Klicken Sie auf den einfachen Pfeil, um die Terminbeschreibung zu lesen.' align='texttop'><b>&nbsp;&nbsp;Termine fÅr die Zeit vom ".strftime("%d. %B %Y", $date_start)." bis zum ".strftime("%d. %B %Y", $date_end)."</b></td></tr>";
			else
...
function show_personal_dates ($range_id, $date_start, $date_end, $show_docs=FALSE, $show_admin=FALSE, $open){
	...
	if($list->existTermin()){
		...
		if($show_admin){
			$colspan++;
			echo "\n<tr><td class='topic' width=\"99%\">&nbsp;<img src='./pictures/icon-uhr.gif' border='0' alt='Termine. Klicken Sie rechts auf die Pfeile, um Termine in diesen Bereich zu bearbeiten. Klicken Sie auf den einfachen Pfeil, um die Terminbeschreibung zu lesen.' align='absmiddle'><b>&nbsp;&nbsp;Termine fÅr die Zeit vom ".strftime("%d. %B %Y", $list->getStart())." bis zum ".strftime("%d. %B %Y", $list->getEnd())."</b></td>";
			echo "\n<td align='right' class='topic'>&nbsp;$admin_link<img src='./pictures/pfeillink.gif' border='0' alt='Termine bearbeiten'></a>&nbsp;</td></tr>";
		}
		else
			echo "\n<tr><td class='topic'>&nbsp;<img src='./pictures/icon-uhr.gif' border='0' alt='Termine. Klicken Sie auf den einfachen Pfeil, um die Terminbeschreibung zu lesen.' align='absmiddle'><b>&nbsp;&nbsp;Termine fÅr die Zeit vom ".strftime("%d. %B %Y", $list->getStart())." bis zum ".strftime("%d. %B %Y", $list->getEnd())."</b></td></tr>";
		echo "\n";
		...
		while($termin = $list->nextTermin()){
			...
			// Zur Identifikation von auf- bzw. zugeklappten Terminen muss zusÑtzlich
			// die Startzeit ÅberprÅft werden, da die Wiederholung eines Termins die
			// gleiche ID besitzt.
