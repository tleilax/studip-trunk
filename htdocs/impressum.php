...
<? IF ($view=="main") {?>
	...
	<tr>
		...
		Weitere Informationen finden sie auf <a target="_new" href="http://www.studip.de">www.studip.de</a><br /><br />
		</td>
	<td class="blank" align = right><a target="_new" href="http://www.studip.de"><img src="pictures/studipanim.gif" border="0"></a><br /><b>Version: </b><? echo $SOFTWARE_VERSION?>&nbsp; &nbsp; &nbsp; &nbsp; 
	...
	<br><b>AndrÇ Noack</b>, eMail: <a href="mailto:andre.noack@gmx.net">andre.noack@gmx.net</a> (Kernentwicklung, Newsverwaltung, Chat)
...
IF ($view=="statistik") {?>
	...
	$anzahl = $db->num_rows();
	echo "<tr><td>Aktive Veranstaltungen:</td><td align=right>$anzahl</td></tr>"; 
	
	$db->query("SELECT * from archiv");
	$anzahl = $db->num_rows();
	echo "<tr><td nowrap>Archivierte Veranstaltungen:</td><td align=right>$anzahl</td></tr>"; 

	echo "<tr><td colspan=2>&nbsp; </td></tr>";

	$db->query("SELECT * FROM Institute");
	$anzahl = $db->num_rows();
	echo "<tr><td>beteiligte Institute:</td><td align=right>$anzahl</td></tr>"; 

	$db->query("SELECT * FROM Fakultaeten");
	$anzahl = $db->num_rows();
	echo "<tr><td>beteiligte Fakult&auml;ten:</td><td align=right>$anzahl</td></tr>"; 

	echo "<tr><td colspan=2>&nbsp; </td></tr>";

	$db->query("SELECT * from auth_user_md5 WHERE perms='admin'");
	$anzahl = $db->num_rows();
	echo "<tr><td>registrierte Administratoren:</td><td align=right>$anzahl</td></tr>"; 
	
	$db->query("SELECT * from auth_user_md5 WHERE perms='dozent'");
	$anzahl = $db->num_rows();
	echo "<tr><td>registrierte Dozenten:</td><td align=right>$anzahl</td></tr>"; 
	
	$db->query("SELECT * from auth_user_md5 WHERE perms='tutor'");
	$anzahl = $db->num_rows();
	echo "<tr><td>registrierte Tutoren:</td><td align=right>$anzahl</td></tr>"; 
	
	$db->query("SELECT * from auth_user_md5 WHERE perms='autor'");
	$anzahl = $db->num_rows();
	echo "<tr><td>registrierte Studierende:</td><td align=right>$anzahl</td></tr>"; 
	
	echo "<tr><td colspan=2>&nbsp; </td></tr>";
	
	$db->query("SELECT * from px_topics");
	$anzahl = $db->num_rows();
	echo "<tr><td>Postings:</td><td align=right>$anzahl</td></tr>"; 	
	
	$db->query("SELECT * from dokumente");
	$anzahl = $db->num_rows();
	echo "<tr><td>Dateien:</td><td align=right>$anzahl</td></tr>"; 	
	
	$db->query("SELECT * from literatur");
	$anzahl = $db->num_rows();
	echo "<tr><td>Literaturlisten:</td><td align=right>$anzahl</td></tr>"; 	
	
	$db->query("SELECT * from termine");
	$anzahl = $db->num_rows();
	echo "<tr><td>Termine:</td><td align=right>$anzahl</td></tr>"; 	
	
	$db->query("SELECT * from news");
	$anzahl = $db->num_rows();
	echo "<tr><td>News:</td><td align=right>$anzahl</td></tr></table>"; 	
	
	echo "</td><td class=blank align=center><table class=blank width=100% border=0><tr><td class=blank><a target='_new' href='http://www.studip.de'><img src='pictures/studipanim.gif' border=0></a></td></tr></table></td></tr>";
	
	?>		
	<td colspan=2 class=blank align=center><br><br><table width=80% cellpadding=0 cellspacing=0 class=blank>	
...
IF ($view=="ansprechpartner") {?>	
	...
	<tr>
		<td class="blank" colspan=2><br><br>
...
<?
	...
	while ($db->next_record())
		...
		if (($count<$half) && ($change==0) && ($last_inst<>$db->f("Institut_id")))
			{
			echo"<br><br></td><td valign=\"top\" width=\"45%\">";
			$change=1;
			}
		if ($last_inst<>$db->f("Institut_id"))
			{
			$inst_id=$db->f("Institut_id");
//			echo "<br><br><b>".htmlReady($db->f("Name")).":</b><br>";
			echo "<br><br><b><a href=\"institut_main.php?auswahl=".$db->f("Institut_id")."\">".htmlReady($db->f("Name"))."</a>:</b><br>"; //EXPERIMENTELL!!
			}
		$last_inst=$inst_id;
		echo "<a href=\"about.php?username=".$db->f("username")."\">".$db->f("Vorname")." ".$db->f("Nachname")."</a>, eMail: <a href=\"mailto:".$db->f("Email")."\">".$db->f("Email")."</a><br>";
		$count=$count-1;
...
echo "Stud IP ist ein Open-Source Projekt und steht unter der GNU General Public License. SÑmtliche zum Betrieb notwendigen Dateien k˜nnen unter <a href='http://sourceforge.net/projects/studip/'>http://sourceforge.net/projects/studip/</a> heruntergeladen werden. ";
echo "<br><br>Die technische Grundlage bietet ein LINUX-System mit Apache Webserver sowie eine MySQL Datenbank, die Åber PHP gesteuert wird.";
echo "<br><br>Im System findet ein 6-stufiges Rechtesystem Verwendung, das individuell auf verschiedenen Ebenen wirkt - etwa in Veranstaltungen, Einrichtungen, FakultÑten oder systemweit.";
echo "<br><br>Seminare oder Arbeitsgruppen k˜nnen mit Passw˜rtern geschÅtzt werden - die VerschlÅsselung erfolgt mit einem MD5 one-way-hash.";
echo "<br><br>Das System ist zu 100% Åber das Internet administrierbar, es sind keine zusÑtzlichen Werkzeuge n˜tig. Ein Webbrowser ab Version 4 wird empfohlen.";
echo "<br><br>Das System wird stÑndig weiterentwickelt und an die WÅnsche unserer Nutzer angepasst - <a href='mailto:crew@studip.de'>sagen Sie uns Ihre Meinung!</a>";
