            ...
            <form  name="search" method="post" action="<?= URLHelper::getLink() ?>" >
                ...
                <table class="default" border=0 cellspacing=0 cellpadding=2>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= _('Lehrende der Veranstaltung:') ?></td>
                            <td>
...
if ($_SESSION['archiv_data']["perform_search"]) {
    ...
    echo "<tr height=28><td  width=\"1%\" class=\"table_header\">&nbsp;</td>\n";
        ...
        echo "<td  width=\"20%\" class=\"table_header\" align=center valign=bottom><b><a href=\"". URLHelper::getLink("?sortby=dozenten") ."\">" . _("Lehrende") . "</a></b></td>\n";
        ...
        foreach ($results as $result) {
            ...
            if ($_SESSION['archiv_data']["open"]==$result['seminar_id']) {
                ...
                echo Icon::create('arr_1down', 'clickable', ['title' => _('Zuklappen')) + array('valign' => 'top'])->asImg(16);
                echo "</a></td>";
                ...
                echo Icon::create('arr_1right', 'clickable', ['title' => _('Aufklappen')) + array('valign' => 'top'])->asImg(16);
                echo "</a></td>";
        ...
        echo "<td align=center class=\"$class\">&nbsp;<font size=-1>".htmlReady($result['dozenten'])."</font></td>";
            ...
            if (archiv_check_perm($result['seminar_id']))
                $view = 1;
            if ($view == 1) {
                echo "<td class=\"$class\" width=\"3%\">&nbsp;<a href=\"". URLHelper::getLink("?dump_id=".$result['seminar_id']) ."\" target=_blank>" .  Icon::create('info', 'clickable')->asImg(16, ['class' => 'text-top', 'title' =>_('Komplettansicht')]) . "</a></td>";
                echo "<td class=\"$class\" width=\"3%\" style=\"white-space: nowrap\">&nbsp;";
                if (!$result['archiv_file_id']=='') {
                    echo '<a href="' . URLHelper::getLink(GetDownloadLink($result['archiv_file_id'], $file_name, 1)) .'"> ' .  Icon::create('download', 'clickable')->asImg(16, ['class' => 'text-top', 'title' =>_('Dateisammlung')]) . '</a>';
                }
                if ($result['archiv_protected_file_id'] && in_array(archiv_check_perm($result['seminar_id']), words("tutor dozent admin"))) {
                    echo '<a href="' . URLHelper::getLink(GetDownloadLink($result['archiv_protected_file_id'], _("Geschützte-") . $file_name, 1)) .'"> ' .  Icon::create('download', 'clickable')->asImg(16, ['class' => 'text-top', 'title' =>_('Geschützte Dateisammlung')]) . '</a>';
                }
                ...
                if (archiv_check_perm($result['seminar_id']) == "admin") {
                    ...
                    echo Icon::create('trash', 'clickable', ['title' => _('Diese Veranstaltung aus dem Archiv entfernen')])->asImg(16);
                    echo '</a>';
                }
                echo "</td>";
            } else
                ...
                if ($_SESSION['archiv_data']["edit_grants"]) {
                    ...
                    while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        if ($row['status'] != "dozent") {
                            ...
                            echo Icon::create('trash', 'clickable', ['title' => _('Dieser Person die Zugriffsberechtigung entziehen')])->asImg(16);
                            echo '</a>';
                        }
                        echo "<br>";
                    }
