$( document ).ready(function() {
	...
	$("del.diffdel").each(function() {
		...
		$(this).parentsUntil("div").each(function() {
		...
		if (mvv_field != "" ) {
			...
			for (var i = 0; i < fields.length; ++i) {
				...
				if ( obj_elements.length == 1 && mvv_coid){
					var senddata = { "mvv_field": fields[i], "mvv_id": mvv_id, "mvv_coid": mvv_coid, "log_action": mvv_log_action };

				} else if ( fields[i] == "mvv_modulteil_stgteilabschnitt.differenzierung" && mvv_coid){
					var classes = $(this).parent().attr("class").split(' ');
					if (classes.length > 1){
						var mvv_debug = $(this).parent().attr("data-mvv-index") + ";" + classes[1];
						var senddata = { "mvv_field": fields[i], "mvv_id": mvv_id, "mvv_coid": mvv_coid, "log_action": mvv_log_action, "mvv_debug": mvv_debug };
					} else {
						return true;
					}
	...
	$(".mvv-diff-deleted").each(function() {
		$(this).find("table").each(function() {
			...
			var url = STUDIP.URLHelper.getURL("plugins.php/mvvplugin/shared/log_event/get_log_autor");
			$.post( url, { "mvv_field": "mvv_"+mvv_type, "mvv_id": mvv_id, "log_action": "del" }, function( data ) {
			    if (data) {
			    	var info = 'Entfernt von ' + data.user + ' am ' + data.time;
			    	curtable.attr("title",info);
