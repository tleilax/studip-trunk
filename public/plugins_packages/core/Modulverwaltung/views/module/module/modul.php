...
if ($GLOBALS['MVV_MODUL']['SPRACHE']['default'] != $deskriptor->sprache) {
    ...
    <? else : ?>
        <? if ($deskriptor->isNew()) : ?>
            <?= sprintf('Modul: <em>%s</em> in der Ausgabesprache <em>%s</em> neu anlegen.', $modul->getDisplayName(),
                    $GLOBALS['MVV_MODUL_DESKRIPTOR']['SPRACHE']['values'][$deskriptor->sprache]['name']) ?>
        <? else : ?>
...
<form id="modul_form" class="default" action="<?= $controller->url_for('/modul', $modul->id) ?>" method="post">
    ...
    <fieldset>
        ...
        <label id="mvv-field-modul-quelle"><?= _('Quelle') ?>
            ...
            <? else : ?>
            <?= sprintf(_('Dieses Modul ist eine Novellierung des Moduls <strong><em>%s</em></strong>.'), $modul->modul_quelle->getDisplayName()) ?>
        ...
        <? if ($def_lang) : ?>
            <label id="mvv-field-modul-modul_start">
                ...
                <? else : ?>
                    ...
                    <?= htmlReady($sem->name) ?>
                    <input type="hidden" name="start" value="<?= $modul->start ?>">
                <? endif; ?>
            </label>
            ...
            <label id="mvv-field-modul-version">
                <?= _('Version:') ?>
                <input <?= $perm->disable("version") ?> type="text" name="version" id="version" value="<?= $modul->version ?>" maxlength="120">
            </label>
        <? else: ?>
                    ...
                    <label><?= _('Zuordnen mit der Funktion:') ?>
                        <select>
                            <? foreach ($GLOBALS['MVV_MODUL']['PERSONEN_GRUPPEN']['values'] as $gruppe_id => $gruppe) : ?>
                                <option value="<?= $gruppe_id ?>"><?= $gruppe['name'] ?></option>
                            <? endforeach; ?>
                        </select>
                    </label>
                </div>
                        ...
                        <? if (is_array($grouped_users[$gruppe_id])) : ?>
                            <? foreach ($grouped_users[$gruppe_id] as $assigned_user) : ?>
                                <li id="users_<?= $gruppe_id ?>_<?= $assigned_user->user_id ?>" class="sort_items">
                                    ...
                                    <div class="mvv-item-list-buttons">
                                        <a href="#" class="mvv-item-remove"><?= Icon::create('trash', 'clickable', array('title' => _('Person entfernen')))->asImg(); ?></a>
                                    </div>
    ...
    <fieldset>
        ...
        <label id="mvv-field-modul-kompetenzziele"><?= _('Kompetenzziele') ?>
       	<? if($perm_d->haveFieldPerm('kompetenzziele', MvvPerm::PERM_WRITE)): ?>
            <textarea cols="60" rows="5" name="kompetenzziele" id="kompetenzziele" class="add_toolbar ui-resizable"><?= htmlReady($deskriptor->kompetenzziele) ?></textarea>
       	<? else: ?>
        ...
        <? if($perm_d->haveFieldPerm('inhalte', MvvPerm::PERM_WRITE)): ?>
            <textarea cols="60" rows="5" name="inhalte" id="inhalte" class="add_toolbar ui-resizable"><?= htmlReady($deskriptor->inhalte) ?></textarea>
        <? else: ?>
        ...
        <? if($perm_d->haveFieldPerm('literatur', MvvPerm::PERM_WRITE)): ?>
            <textarea cols="60" rows="5" name="literatur" id="literatur" class="add_toolbar ui-resizable"><?= htmlReady($deskriptor->literatur) ?></textarea>
        <? else: ?>
        ...
        <? if($perm_d->haveFieldPerm('links', MvvPerm::PERM_WRITE)): ?>
            <textarea cols="60" rows="5" name="links" id="links" class="add_toolbar ui-resizable"><?= htmlReady($deskriptor->links) ?></textarea>
        <? else: ?>
    ...
    <fieldset id="mvv-field-modul-kommentar_note">
        ...
        <? if($perm_d->haveFieldPerm('kommentar_note', MvvPerm::PERM_WRITE)): ?>
            <textarea cols="60" rows="5" name="kommentar_note" id="kommentar_note" class="add_toolbar ui-resizable"><?= htmlReady($deskriptor->kommentar_note) ?></textarea>
        <? else: ?>
    ...
    <fieldset>
        ...
        <? if ($perm_d->haveFieldPerm('ersatztext', MvvPerm::PERM_WRITE)) : ?>
            <textarea cols="60" rows="5" name="ersatztext" id="ersatztext" class="add_toolbar ui-resizable"><?= htmlReady($deskriptor->ersatztext) ?></textarea>
        <? else : ?>
    ...
    <fieldset id="mvv-field-modul-status">
        ...
        <? if ($def_lang) : ?>
            ...
            <label id="mvv-field-modul-kommentar_status"><?= _('Kommentar:') ?>
                ...
                <textarea cols="60" rows="5" name="kommentar_status" id="kommentar_status" class="add_toolbar ui-resizable"><?= htmlReady($modul->kommentar_status) ?></textarea>
                <? else : ?>
    ...
    <input type="hidden" name="display_language" value="<?= $this->display_language ?>">
    <footer>
    <? if ($deskriptor->isNew()) : ?>
