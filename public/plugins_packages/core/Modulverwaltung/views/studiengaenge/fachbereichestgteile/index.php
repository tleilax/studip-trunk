    ...
    <tr class="header-row">
        <td class="toggle-indicator">
            <? if ($fachbereich['stgteile']) : ?>
                <a class="mvv-load-in-new-row" href="<?= $controller->url_for('/details_fachbereich', $fachbereich['institut_id']) ?>"><?= htmlReady($fachbereich['name']) ?></a>
            <? else: ?>
        ...
        <? if (MvvPerm::havePermCreate('StudiengangTeil')) : ?>
            <a href="<?= $controller->url_for('/stgteil_fachbereich', $fachbereich['institut_id']) ?>">
                <?= Icon::create('file+add', 'clickable', array('title' => _('Neuen Studiengangteil in diesem Fachbereich anlegen')))->asImg(); ?>
            </a>
        <? endif; ?>
        </td>
    <? if ($details_id == $fachbereich['institut_id'] || sizeof($stgteil_ids)) : ?>
    <? $stgteile = StudiengangTeil::findByFachbereich($fachbereich['institut_id'], array('stgteil_id' => $stgteil_ids), 'fach_name,zusatz,kp', 'ASC'); ?>
