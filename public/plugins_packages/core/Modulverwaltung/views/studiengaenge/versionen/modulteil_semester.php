...
<form class="default" data-dialog="" action="<?= $controller->url_for('/modulteil_semester', $abschnitt_modul->abschnitt_id, $modulteil->id) ?>" method="post">
    <?= CSRFProtection::tokenTag() ?>
    <fieldset>
        <legend><?= _('Belegung in folgenden Fachsemestern') ?></legend>
        <? for ($i = 1; $i <= $GLOBALS['MVV_MODULTEIL_FACHSEMESTER']; $i++) : ?>
        <? $fachsemester = $abschnitt_modul->getFachsemester($modulteil->getId(), $i) ?>
