...
<form class="default" action="<?= $controller->url_for('/stgteil', $stgteil->id) ?>" method="post">
    ...
    <fieldset>
        ...
        <label>
            ...
            <input <?= $perm->disable('zusatz')?> type="text" name="zusatz" id="stgteil_zusatz" maxlength="200" value="<?= htmlReady($stgteil->zusatz) ?>">
        </label>
        <label>
            <img src="<?= Assets::image_path('languages/lang_en.gif') ?>" alt="<?= _('Englisch') ?>">
            <input <?= $perm->disable('zusatz_en')?> type="text" name="zusatz_en" id="stgteil_zusatz_en" maxlength="200" value="<?= htmlReady($stgteil->zusatz_en) ?>">
        </label>
    </fieldset>
    <fieldset>
        ...
        <? if ($perm->haveFieldPerm('fachberater')) : ?>
            ...
            <? if (Request::submitted('search_fachberater')) : ?>
                <?= Icon::create('refresh', 'clickable', ['name' => 'reset_fachberater', 'data-qs_id' => $search_fachberater_id])->asInput(); ?>
            <? else : ?>
                <?= Icon::create('search', 'clickable', ['name' => 'search_fachberater', 'data-qs_id' => $search_fachberater_id, 'data-qs_name' => $search_fachberater->getId(), 'class' => 'mvv-qs-button'])->asInput(); ?>
            <? endif; ?>
        <? endif; ?>
