...
class PrototypeHelper {
      ...
      case 'update':
        $js_func = $options['position']
                   ? sprintf("new Insertion.%s('%s','%s')",
                             Trails_Inflector::camelize($options['position']),
                             $element_id, $content)
  ...
  function options_for_ajax($options) {
    ...
    if (isset($options['position']))
      $js_opt['insertion'] = 'Insertion.'.Trails_Inflector::camelize($options['position']);
  ...
  function get_callbacks() {
    static $callbacks;
    if (!$callbacks)
      $callbacks = array('uninitialized', 'loading', 'loaded', 'interactive',
...
class Trails_JavascriptGenerator {
...
################################################################################
   ...
   * @return type <description>
   */
  function insert_html($position, $id, $options_for_render) {
    $insertion = Trails_Inflector::camelize($position);
    $this->call('new Insertion.'.$insertion, $id,
                $this->render($options_for_render));
  }
  ...
  function replace_html($id, $options_for_render) {
    $this->call('Element.update', $id, $this->render($options_for_render));
  }
   ...
   */
   function replace($id, $options_for_render) {
     $this->call('Element.replace', $id, $this->render($options_for_render));
   }
  ...
  function redirect_to($location) {
    $this->assign('window.location.href', UrlHelper::url_for($location));
  }
  ...
  function visual_effect($name, $id = FALSE, $js_opt = array()) {
    Helper::use_helper('scriptaculous');
   ...
   * @ignore
   */
  function render($options_for_render) {
    if (is_array($options_for_render)) {
      list($template, $attr, $layout) = $options_for_render;
      return Trails_Template::render_template($template, $attr, $layout);
    }
    else
      return $options_for_render;
  }
  ...
  function javascript_object_for($object) {
    return Trails_Toolkit::to_json($object);
  }
