        ...
        <tr class="header-row" id="fachbereich_<?= $fachbereich->id ?>">
            <td class="toggle-indicator">
                <? if (is_null($fachbereich->name) && $fachbereich->count_objects) : ?>
                    <a class="mvv-load-in-new-row" href="<?= $controller->url_for('/details', $fachbereich->id) ?>"><?= _('Keinem Fachbereich zugeordnet') ?></a>
                <? else : ?>
                    <? if ($fachbereich->count_objects) : ?>
                    <a class="mvv-load-in-new-row" href="<?= $controller->url_for('/details', $fachbereich->id) ?>"><?= htmlReady($fachbereich->getDisplayName()) ?> </a>
                    <? else : ?>
