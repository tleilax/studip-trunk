<form data-dialog="reload-on-close" name="approve" action="<?= $controller->url_for('/approve', $studiengang_id) ?>" method="post" style="margin-left: auto; margin-right: auto;">
<?= $this->render_partial('shared/studiengang/_studiengang', array('studiengang' => $studiengang, 'institut' => $institut)); ?>
<? if (count($studiengang->stgteil_assignments)) : ?>
    ...
    <? foreach($studiengang->stgteil_bezeichnungen as $stgteilbez) : ?>
        ...
        <? foreach( StudiengangStgteil::findByStudiengangStgteilBez($studiengang->getId(),$stgteilbez->getId()) as $stgstgteil) : ?>
            ...
            <? if($stgteil) : ?>
                <?= $this->render_partial('shared/studiengang/_studiengangteil', array('stgteil' => $stgteil )); ?>
                <? foreach (StgteilVersion::findByStgteil($stgteil->getId()) as $version) : ?>
                    <? if ($version->stat == 'genehmigt') : ?>
                        <?= $this->render_partial('shared/studiengang/_stgteilversion', array('version' => $version)) ?>
                    <? endif; ?>
                <? endforeach; ?>
            <? endif; ?>
        <? endforeach; ?>
    ...
    <? foreach($studiengang->studiengangteile as $stgteil) :?>
        <?= $this->render_partial('shared/studiengang/_studiengangteil', array('stgteil' => StudiengangTeil::find($stgteil->stgteil_id) )); ?>
    <? endforeach;?>
    <? endif; ?>
<? endif; ?>
