                ...
                <tr class="header-row" id="abschnittt_<?= $abschnitt->id ?>">
                    <td class="toggle-indicator">
                        <a class="mvv-load-in-new-row"
                           href="<?= $controller->url_for('/details_abschnitt', $abschnitt->id) ?>"><?= htmlReady($abschnitt->name) ?></a>
                    </td>
                        ...
                        <form method="post">
                            ...
                            <? if (MvvPerm::havePermWrite($version)) : ?>
                                <? $actionMenu->addLink(
                                        ...
                                        Icon::create('edit', 'clickable', tooltip2(_('Studiengangteil-Abschnitt bearbeiten'))),
                                        ['data-dialog' => true])
                                ?>
                            <? endif; ?>
                            <? if (MvvPerm::haveFieldPermAbschnitte($version, MvvPerm::PERM_CREATE)) : ?>
                                <? if (!$abschnitt->count_module) : ?>
                                    <? $actionMenu->addButton(
                                        ...
                                        Icon::create('trash', 'clickable', tooltip2(_('Studiengangteil-Abschnitt löschen'))),
                                        [
