    ...
    <thead>
        <tr>
            <th colspan="4"><?=_('Individuelle-Einstellungen für: ')?>
                ...
                <br><span style="font-size: 75%;">Aktuell verwendete Einstellung: <?=$this->viewData['groupConfig']?></span>
            </th>            
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>
                ...
                <input id="upload" name="upload" value="<? if(count($this->viewData['userConfig'])>0 ):?><?=$this->viewData['userConfig']['upload']?>
                    <?else :?> 0 <?endif;?>">
            ...
            <td>
                ...
                <input id="quota" name="quota" value="<? if(count($this->viewData['userConfig'])>0 ): ?><?=$this->viewData['userConfig']['quota']?> <?else : ?> 0 <?endif;?>">
                <select id="unitQuota" name="unitQuota">
                    <? foreach(array( 'kB','MB','GB','TB') as $unitQuota) : ?>
                        <option value="<?= $unitQuota ?>"
                            <?if($unitQuota=='MB' || (count($this->viewData['userConfig'])>0 && $unit==$this->viewData['userConfig']['quota_unit'])) :?>
                                selected <?endif;?>><?= $unitQuota ?></option>
                    <? endforeach ?>
                </select>
            </td>
            <td colspan="2">
                <label><?=_('Upload untersagen')?></label><br>
                <input type="checkbox" id="forbidden" name="forbidden" 
                    <?if(count($this->viewData['userConfig'])>0 && $this->viewData['userConfig']['forbidden']==1): ?> checked <?endif;?>>
            </td>
        </tr>
        <tr>
            <td colspan="4">
            ...
            <td colspan="4">
             <label><?=_('Dateibereich sperren')?></label><br>
                <input type="checkbox" id="close" name="close" 
                    <?if(count($this->viewData['userConfig'])>0 && $this->viewData['userConfig']['area_close']==1):?>
                       checked<?endif;?>>   
            </td>
            </tr>
            <tr>
            <td colspan="4">
                <label><?=_('Bemerkung zum Sperren des Dateibereichs')?></label><br>
            <textarea name="closeText" cols="50" rows="10" 
                      style="resize: none;" align="left" >
            <?if(count($this->viewData['userConfig'])>0 &&
                    strlen($this->viewData['userConfig']['area_close_text'])>1) : ?><?= trim($this->viewData['userConfig']['area_close_text'])?>
            <? endif;?>
            </textarea>  
            </td>
        </tr>
