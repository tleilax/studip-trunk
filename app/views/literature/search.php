    ...
    <p style="text-align: right">
        ...
        <? if ($search->start_result > 1) : ?>
            <a href="<?= URLHelper::getLink('', array('change_start_result' => ($search->start_result - 5))) ?>">
                <?= Icon::create('arr_2left', 'clickable')->asImg(16, ['hspace' => 3]); ?>
            </a>
        <? endif ?>
        <?= $search->start_result . " - " . $end_result; ?>
        <? if ($search->start_result + 4 < $num_hits) : ?>
            <a href="<?= URLHelper::getLink('', array('change_start_result' => ($search->start_result + 5))) ?>">
                <?= Icon::create('arr_2right', 'clickable')->asImg(16, ['hspace' => 3]); ?>
            </a>
        <? endif ?>
    </p>
            ...
            <section class="contentbox">
                <header>
                    <h1>
                        ...
                        <? else : ?>
                            <? $addon = "<a href=\"$link\">"; ?>
                            <? $addon .= Icon::create('exclaim', 'clickable', ['title' => _('Eintrag in Merkliste aufnehmen')])->asImg(16); ?>
                            <? $addon .= "</a>"; ?>
                        <? endif ?>
