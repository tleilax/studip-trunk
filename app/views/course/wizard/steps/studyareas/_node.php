...
<li class="sem-tree-<?= htmlReady($node->id) ?> keep-node" data-id="<?= $node->id ?>">
    ...
    <?= Icon::create('arr_2left', 'sort')->asInput(["name" => 'assign['.$node->id.']', "onclick" => "return STUDIP.CourseWizard.assignNode('".$node->id."')", "class" => in_array($node->id,$values['studyareas']?:array())?'hidden-no-js':'', "style" => in_array($node->id,$values['studyareas']?:array())?'display:none':false]) ?>
    <?php endif ?>
