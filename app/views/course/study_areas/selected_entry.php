...
<li id="study_area_selection_<?= $_id ?>" class="<?= TextHelper::cycle('odd', 'even') ?>">
  ...
  <a onClick="STUDIP.study_area_selection.expandSelection('<?= $_id ?>','<?= htmlReady($course_id) ?>');return false;"
     href="<?= URLHelper::getLink(isset($url) ? $url : '', array('study_area_selection[selected]' => $area->getID())) ?>">
