<form class="default" action="<?= $controller->url_for('course/grouping/action') ?>" method="post" data-dialog="size=auto">
        ...
        <article class="studip toggle" id="<?= $child->id ?>">
            <header>
                <h1>
                    ...
                    <a href="<?= ContentBoxHelper::href($child->id, ['contentbox_type' => 'news']) ?>" data-course-id="<?= $child->id ?>"
    ...
    <? endforeach ?>
    <? if (count($parentOnly) > 0) : ?>
        <article class="studip toggle" id="<?= $course->id ?>">
            <header>
                <h1>
                    <a href="<?= ContentBoxHelper::href($course->id, ['contentbox_type' => 'news']) ?>" data-course-id="<?= $course->id ?>"
