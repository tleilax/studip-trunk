<form class="default" action="<?= $controller->url_for('course/wizard/step') ?>" method="post" data-dialog>
    ...
    <section>
        ...
        <input type="number" name="batch[number]" value="5" min="1" required>
    </section>
    <section>
        <span class="required">
        ...
        <label>
            <input type="radio" name="batch[add_number_to]" value="name" required>
            <?= _('Name der Veranstaltung') ?>
        </label>
        <label>
            <input type="radio" name="batch[add_number_to]" value="number" required>
    ...
    <section>
        ...
        <label>
            <input type="radio" name="batch[numbering]" value="number" required>
            <?= _('Zahlen (1, 2, 3, ...)') ?>
        </label>
        <label>
            <input type="radio" name="batch[numbering]" value="letters" required>
            <?= _('Buchstaben (A, B, C, ...)') ?>
        </label>
    </section>
    <footer data-dialog-button>
        <input type="hidden" name="batch[parent]" value="<?= $course->id ?>">
