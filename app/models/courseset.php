...
class CoursesetModel {
    ...
    public function getInstCourses($instituteIds, $coursesetId='', $selectedCourses=array(), $semester_id = null, $filter = false) {
        ...
        $excludeTypes = array_merge(
            studygroup_sem_types() ?: array(''),
            SemType::getGroupingSemTypes() ?: array('')
        );
        ...
        if ($filter === true) {
            $query = "SELECT su.`Seminar_id` FROM `seminar_user` su
                ...
                AND su.`user_id`=?";
            $parameters = array($excludeTypes, $currentSemester->beginn, $currentSemester->beginn, $GLOBALS['user']->id);
            ...
            $courses = $db->fetchFirst("SELECT DISTINCT s.seminar_id FROM seminare s
                ...
                WHERE s.status NOT IN(:exclude_types) AND s.start_time <= :sembegin AND (:sembegin <= (s.start_time + s.duration_time) OR s.duration_time = -1)
                AND s.Institut_id IN(:institutes)
                AND (s.name LIKE :filter OR s.Veranstaltungsnummer LIKE :filter OR Nachname LIKE :filter)",
                     array('exclude_types' => $excludeTypes,
